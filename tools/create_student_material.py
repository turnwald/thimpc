"""Create student-facing study notebooks from solution walkthroughs."""
...
def student_notebooks() -> list[Path]:
    return sorted(STUDIES_DIR.glob(f"study_*/{STUDENT_NOTEBOOK_NAME}"))
...
def main() -> None:
    notebooks = study_notebooks()
    if not notebooks:
        print(f"no {SOLUTION_NOTEBOOK_NAME} files found under {STUDIES_DIR}; nothing to generate")
        return
